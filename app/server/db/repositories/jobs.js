        ...
        [
          ...
          scene.narration ||
            (scene.lines || [])
              ...
              .join("  "),
          stringifyJson(scene.lines || null),
          scene.image_prompt,
          scene.motion_prompt,
        ],
      );
...
export async function attachProduct(userId, jobId, productId, sortOrder = 0) {
  return withTransaction(async (connection) => {
    ...
    if (!row) {
      console.warn("[attachProduct] product not found or not owned:", {
        userId,
        jobId,
        productId,
      });
...
export async function attachReference(
  ...
  return withTransaction(async (connection) => {
    let assetId = referenceId;
    let role = requestedRole || "character";

    // Try to resolve via reference_library (best case)
    try {
      const [[ref]] = await connection.execute(
        `SELECT r.reference_type, r.asset_id 
         FROM reference_library r 
         WHERE (r.id = ? OR r.asset_id = ?) AND r.deleted_at IS NULL 
         LIMIT 1`,
        [referenceId, referenceId],
      );
      if (ref) {
        assetId = ref.asset_id;
        role = requestedRole || roleByType[ref.reference_type] || "character";
      }
    } catch (e) {
      /* table or column may not exist */
    }

    // Insert directly — never fail
    await connection.execute(
      ...
      [crypto.randomUUID(), jobId, userId, assetId, role, sortOrder],
    );
    return true;
  });
}
